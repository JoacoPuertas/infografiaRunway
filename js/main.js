...
renderer.setClearColor(0x000000); // Color de fondo
...
function hideOverlay() {
  overlayContainer.classList.add("fade-out"); // Agrega la clase de desvanecimiento
  overlayContainer.classList.add("hide"); // Agrega la clase para cambiar a opacidad 0
}
overlayContainer.addEventListener("click", () => {
  overlayContainer.style.display = "none"; // Oculta inmediatamente sin animación
});
...
function showOverlay() {
  ...
  setTimeout(() => {
    hideOverlay();
  }, 3500); // Cambia 5000 a la cantidad de tiempo que desees
}
...
function movFinal() {
  ...
  function handleMouseMove(event) {
    ...
    if (intersects.length > 0) {
      ...
      // Restaurar el cursor a default si no está sobre ningún objeto
      document.body.style.cursor = "default";
    }
  }
  ...
  const imageModalIAs = document.getElementById("image-modal-ias");
  const closeModalButtonIAs = document.getElementById("close-modal-button-ias");
  const imageButton = document.getElementById("imageButtonIAs");

  // Función para mostrar el modal de imagen
  function showImageModalIAs() {
    imageModalIAs.style.display = "flex"; // Mostrar el modal
  ...
  function createImageButtonIAs() {
    if (!imageButtonIAs) {
      // Solo crea el botón si no existe
      imageButtonIAs = document.createElement("img");
      imageButtonIAs.src = "../assets/botonIAs.png"; // Imagen del botón
      imageButtonIAs.style.position = "absolute";
      imageButtonIAs.style.cursor = "pointer";
      imageButtonIAs.style.zIndex = 2000; // Asegúrate de que esté sobre todo
      imageButtonIAs.style.left = "50%"; // Ajustar según sea necesario
      imageButtonIAs.style.top = "20%"; // Ajustar según sea necesario
      imageButtonIAs.style.transform = "translate(-50%, -50%)"; // Centrar la imagen
      document.body.appendChild(imageButtonIAs);

      // Agregar el manejador de clics para el botón-imagen
      imageButtonIAs.addEventListener("click", showImageModalIAs);
    } else {
      imageButtonIAs.style.display = "block"; // Asegúrate de que el botón esté visible
    }
  }
  ...
  function createZoneButtons(zone) {
    ...
    if (zone === zones[0]) {
      // Crear los botones para zone1
      boton1();
      boton2();
      zoneText.innerHTML = "Usos de Runway en la Industria "; // Texto específico para zone1
      zoneText.style.left = "50%";

    } else if (zone === zones[1]) {
      // Crear el botón-imagen para zone2
      createImageButtonIAs();
      zoneText.innerHTML = " Comparación con otras IA Generativas de Imagen"; // Texto específico para zone1
      zoneText.style.left = "50%";

    } else if (zone === zones[2]) {
      // Crear los botones para zone3
      boton3();
      boton4();
      boton5();
      zoneText.innerHTML = "Nacimiento, entrenamiento, inversión y evolución."; // Texto específico para zone1
zoneText.style.left = "30%";
    } else {
      clearZoneButtons();
    }
  }

  // Llamar a botonvolver una vez, ya que no es necesario llamarlo en cada zona
  botonvolver();
  ...
  function clearZoneButtons() {
    ...
    zoneButtons = [];
    // Limpiar el texto cuando se sale de una zona
    const zoneText = document.getElementById("zone-text");
    zoneText.innerHTML = ""; // Limpiar el texto
    // Ocultar botones específicos si existen
    if (imageButtonIAs) {
      imageButtonIAs.style.display = "none"; // Solo ocultar el botón
    }
    if (button1) {
